...
if __name__ == "__main__":
    ...
    if "YOUR_" not in example_config["client_id"]:
        ...
        print("=== 사용 예시 코드 ===")
        example_usage = '''
# 1. 서비스 초기화 (정확한 URL 사용)
service = KakaoBizMessageService(
    base_url="bizmsg-web.kakaoenterprise.com",       # 일반 업종
    # base_url="bizmsg-bank.kakaoenterprise.com",    # 금융권
    # base_url="bizmsg-stock.kakaoenterprise.com",   # 증권
    # base_url="bizmsg-gov.kakaoenterprise.com",     # 공공기관
...
sender = create_kakao_sender(
    base_url="bizmsg-web.kakaoenterprise.com",       # 업종에 맞는 URL 선택
    client_id="실제_클라이언트_ID",
    client_secret="실제_클라이언트_시크릿",
    sender_key="실제_발신프로필키", 
    sender_no="02-1234-5678"
...
# 5. 스테이징 테스트 (실제 메시지 발송되므로 주의!)
test_service = KakaoBizMessageService(
    base_url="stg-user.bizmsg.kakaoenterprise.com",  # 스테이징 URL
    client_id="실제_클라이언트_ID",
    client_secret="실제_클라이언트_시크릿"
)
        '''
