...
def test_analyze_message_with_mock():
    ...
    # classify_category_and_type 가짜 응답
    ...
    # 두 메서드 동시에 Mock 처리
    with patch.object(searcher, "classify_category_and_type", new=AsyncMock(return_value=fake_classify_response)), \
         patch.object(searcher, "extract_message_fields", new=AsyncMock(return_value=fake_extract_response)):

        result = asyncio.run(
            searcher.analyze_message(
