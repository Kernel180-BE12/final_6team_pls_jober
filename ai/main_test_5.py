...
def test_oauth_and_message():
    ...
    # 설정값 - 실제 값으로 변경 필요
    CONFIG = {
        "base_url": "bizmsg-web.kakaoenterprise.com",      # 또는 해당 환경 URL
        "client_id": "YOUR_CLIENT_ID_HERE",                # OAuth 클라이언트 ID
        "client_secret": "YOUR_CLIENT_SECRET_HERE",        # OAuth 클라이언트 시크릿
        "sender_key": "YOUR_SENDER_KEY_HERE",              # 발신 프로필 키
        "sender_no": "02-111-2222",                        # 발신번호
...
if __name__ == "__main__":
    ...
    example_config = {
        ...
        "client_id": "YOUR_CLIENT_ID_HERE",        # 실제 클라이언트 ID로 변경
        "client_secret": "YOUR_CLIENT_SECRET_HERE", # 실제 클라이언트 시크릿으로 변경
        "sender_key": "YOUR_SENDER_KEY_HERE",       # 실제 발신 프로필 키로 변경
        "sender_no": "02-1234-5678",                # 실제 발신번호로 변경
    ...
    if "YOUR_" not in example_config["client_id"]:
        ...
        print("=== 사용 예시 코드 ===")
        example_usage = '''
# 1. 서비스 초기화 (실제 제공받은 서버 호스트 사용)
service = KakaoBizMessageService(
    base_url="실제_제공받은_서버주소.com",        # 예: "api.your-provider.com"
...
import requests
try:
    response = requests.get(f"https://실제_제공받은_서버주소.com", timeout=10)
